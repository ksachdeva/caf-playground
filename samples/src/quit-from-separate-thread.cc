// This program checks the behavior if the quit
// was called from a thread that is not managed by CAF
    ...
    return {
        ...
        [=](add_atom, int x) {
          ...
          if (this->_value >= 3) {
            ...
            std::cout << "add_atom_behavior " << std::this_thread::get_id()
                      << std::endl;

            std::thread t([=]() {
              std::cout << "quit called from  " << std::this_thread::get_id()
                        << std::endl;
...
public:
  ASpawnerActor(caf::actor_config &cfg) : caf::event_based_actor(cfg) {
    ...
    set_down_handler([=](caf::down_msg &msg) {
      ...
      std::cout << "set_down_handler " << std::this_thread::get_id()
                << std::endl;
