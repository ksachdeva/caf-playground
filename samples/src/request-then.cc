...
caf::behavior takes_long_time_respond(caf::event_based_actor *self) {
  return {
...
void caf_main(caf::actor_system &system) {
  ...
  auto buddy = self->spawn(takes_long_time_respond);
  ...
  // and see if they respond back before we get result from it or not
  ...
  // for long running behavior 3
  //
  // Run this sample to verify and make sure to wait for few seconds before
  // pressing the enter
